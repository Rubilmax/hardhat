...
import { EXAMPLE_CONTRACT } from "./example-contracts";
import {
  assertIsNotNull,
  assertWithin,
...
describe("custom provider", function () {
  ...
  describe("getSigner", function () {
    ...
    it("should throw if the address doesn't match an account", async function () {
      await assert.isRejected(
        this.env.ethers.provider.getSigner(
  ...
  describe("getBalance", function () {
    ...
    it.skip("should return the balance of a contract", async function () {
      // FVTODO implement
    });
  });
  ...
  describe("getCode", function () {
    ...
    // FVTODO respond to Richard about this
    let contract: any;
    beforeEach(async function () {
      const signer = await this.env.ethers.provider.getSigner(0);
  ...
  describe("broadcastTransaction", function () {
    it("should send a raw transaction", async function () {
      ...
      assert.equal(tx.to, this.env.ethers.ZeroAddress);
  ...
  describe("getLogs", function () {
    ...
    it("should get the logs by address", async function () {
      ...
      const contract1: any = await factory.deploy();
      const contract2: any = await factory.deploy();
    ...
    it("should get the logs by an array of addresses", async function () {
      ...
      const contract1: any = await factory.deploy();
      const contract2: any = await factory.deploy();
      const contract3: any = await factory.deploy();
  ...
  describe("waitForTransaction", function () {
    it("should get the receipt of an already mined transaction", async function () {
      const signer = await this.env.ethers.provider.getSigner(0);
      const tx = await signer.sendTransaction({ to: signer });

      const receipt = await this.env.ethers.provider.waitForTransaction(
        tx.hash
      );
